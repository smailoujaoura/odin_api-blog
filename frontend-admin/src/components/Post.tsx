            ...
            <article className="max-w-3xl mx-auto px-6 pt-32">
                <header className="mb-12">
                    ...
                    <div className="flex items-center gap-6 text-sm text-zinc-500">
                        <span className="flex items-center gap-2"><User size={16} /> {"shit"}</span>
                        <span className="flex items-center gap-2"><Calendar size={16} /> {new Date(post.createdAt).toLocaleDateString()}</span>
                    </div>
                </header>

                <div className="prose prose-invert prose-sky max-w-none prose-pre:bg-zinc-900 prose-pre:border prose-pre:border-zinc-800">
                ...
                <section className="space-y-8">
                    ...
                    <form onSubmit={handleComment} className="relative">
                        ...
                        <button 
                            type="submit"
                            className="absolute bottom-4 right-4 bg-sky-600 hover:bg-sky-500 p-2 rounded-xl text-white transition-all active:scale-95"
                        >
                            <Send size={20} />
                        </button>
                    </form>
                    ...
                    <div className="space-y-4">
                        {comments.map((comment) => (
                            <div key={comment.id} className="bg-zinc-900/50 border border-zinc-800 p-5 rounded-2xl">
                                <div className="flex justify-between items-start mb-2">
                                    <span className="font-bold text-zinc-200 text-sm">{comment.author.name}</span>
                                    <span className="text-zinc-600 text-[10px]">{new Date(comment.createdAt).toLocaleDateString()}</span>
                                </div>
